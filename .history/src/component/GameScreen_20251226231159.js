...
const GameScreen = ({ roomId, playerId, playerRole, onLeaveRoom }) => {
  ...
  const handleLeaveRoom = async () => {
    ...
    try {
      ...
      // Lấy dữ liệu hiện tại để giữ lại targetCharacter
      const snapshot = await get(roomRef);
      if (snapshot.exists()) {
        const currentData = snapshot.val();
        const currentPlayerData = playerRole === "player1" ? currentData.player1 : currentData.player2;

        // Xóa chỉ id và eliminated, giữ lại targetCharacter
