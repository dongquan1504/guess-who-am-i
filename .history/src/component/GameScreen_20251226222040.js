...
const GameScreen = ({ roomId, playerId, playerRole }) => {
  ...
  const checkWinCondition = async (eliminatedList) => {
    ...
    const remainingCharacter = availableCharacters.find(
      (char) => !eliminatedList.includes(char.id)
    );

    if (!remainingCharacter) return;

    // Lấy target character của đối thủ (nhân vật cần đoán)
    const opponentPlayer = playerRole === "player1" ? gameData.player2 : gameData.player1;
    const correctTarget = opponentPlayer.targetCharacter;

    const roomRef = ref(db, `roomId/${roomId}`);

    // Kiểm tra xem nhân vật còn lại có đúng là target không
    if (remainingCharacter.id === correctTarget.id) {
      // THẮNG!
      await update(roomRef, {
        winner: playerRole,
        status: "finished"
      ...
      setTimeout(() => {
        alert(`🎉 Chúc mừng! Bạn đã đoán đúng! Nhân vật là ${correctTarget.name}!`);
      }, 300);
    } else {
      // THUA!
      const loser = playerRole;
      ...
      setTimeout(() => {
        alert(`❌ Rất tiếc! Bạn đã đoán sai! Nhân vật đúng là ${correctTarget.name}, không phải ${remainingCharacter.name}.`);
      }, 300);
    }
  };
        ...
        <div className="bg-white rounded-xl shadow-lg p-4">
          ...
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="text-gray-600">
              ...
              <span className={`font-semibold ${
                gameData.winner 
                  ? (gameData.winner === playerRole ? 'text-green-600' : 'text-red-600')
                  : 'text-blue-600'
              }`}>
                {gameData.winner 
                  ? (gameData.winner === playerRole ? '🎉 Bạn thắng!' : '😢 Bạn thua!')
                  : (gameData.status === "playing" ? "Đang chơi" : "Đợi người chơi")
                }
              </span>
            </span>
          </div>
          ...
          <div className="mt-2 text-center">
            <span className="text-sm text-gray-600">
              Đã loại: <span className="font-bold text-purple-600">{eliminated.length}/23</span> nhân vật
            </span>
          </div>
        </div>
      </div>
            ...
            {gameData.winner && (
              <div className={`mb-4 p-4 rounded-lg text-center ${
                ...
                <p className={`text-lg font-bold ${
                  gameData.winner === playerRole ? 'text-green-700' : 'text-red-700'
                }`}>
                  {gameData.winner === playerRole 
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
              {availableCharacters.slice(0, 24).map((character) => {
                ...
                return (
                  <div
                    key={character.id}
                    onClick={() => !gameData.winner && handleCharacterClick(character.id)}
                    className={`transition-all duration-300 transform ${
                      !gameData.winner ? 'cursor-pointer hover:scale-105' : 'cursor-not-allowed'
                    } ${
