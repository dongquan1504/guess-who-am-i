...
const GameScreen = ({ roomId, playerId, playerRole }) => {
  ...
  const checkWinCondition = async (eliminatedList) => {
    ...
    if (remainingCharacter.id === opponentTarget.id) {
      ...
      setTimeout(() => {
        alert(`🎉 Chúc mừng! Bạn đã đoán đúng! Nhân vật của đối thủ là ${opponentTarget.name}!`);
      }, 300);
    } else {
      ...
      setTimeout(() => {
        alert(`❌ Rất tiếc! Bạn đã đoán sai! Nhân vật của đối thủ là ${opponentTarget.name}, không phải ${remainingCharacter.name}.`);
      }, 300);
    }
  };
